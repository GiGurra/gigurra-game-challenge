...
const std::string& RequestFromServer::_type_name() {
	static const std::string out("se.gigurra.aichallenge.games.gomoku.RequestFromServer");
	return out;
}

const std::vector<long long>& RequestFromServer::_type_ids() {
	static const std::vector<long long> out = mgen::make_vector<long long>() << 1369439193061359418LL << 7939421773867622329LL << 669011708587813573LL << 1660000543956266627LL;
	return out;
}

const std::vector<short>& RequestFromServer::_type_ids_16bit() {
	static const std::vector<short> out = mgen::make_vector<short>() << -15038 << -5220 << 32152 << -12709;
	return out;
}

const std::vector<std::string>& RequestFromServer::_type_names() {
	static const std::vector<std::string> out = mgen::make_vector<std::string>() << "se.gigurra.aichallenge.Message" << "se.gigurra.aichallenge.GameMessage" << "se.gigurra.aichallenge.games.gomoku.GomokuMessage" << "se.gigurra.aichallenge.games.gomoku.RequestFromServer";
	return out;
}

const std::vector<std::string>& RequestFromServer::_type_ids_16bit_base64() {
	static const std::vector<std::string> out = mgen::make_vector<std::string>() << "xUI" << "65w" << "fZg" << "zls";
	return out;
}

const std::string& RequestFromServer::_type_ids_16bit_base64_string() {
	static const std::string out("xUI65wfZgzls");
	return out;
}

const std::string& RequestFromServer::_type_id_16bit_base64() {
	static const std::string out("zls");
	return out;
}
