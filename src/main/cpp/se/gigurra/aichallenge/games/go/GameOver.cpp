...
const Player& GameOver::getWinner() const {
	return m_winner;
}

const Player& GameOver::getLoser() const {
	return m_loser;
}
...
Player& GameOver::getWinnerMutable() {
	_m_winner_isSet = true;
	return m_winner;
}

Player& GameOver::getLoserMutable() {
	_m_loser_isSet = true;
	return m_loser;
}
...
GameOver& GameOver::setWinner(const Player& winner) {
	m_winner = winner;
	_m_winner_isSet = true;
	return *this;
}

GameOver& GameOver::setLoser(const Player& loser) {
	m_loser = loser;
	_m_loser_isSet = true;
	return *this;
}
...
GameOver& GameOver::_setWinnerSet(const bool state, const mgen::FieldSetDepth depth) {
	if (depth == mgen::DEEP)
		m_winner._setAllFieldsSet(state, mgen::DEEP);
...
GameOver& GameOver::_setLoserSet(const bool state, const mgen::FieldSetDepth depth) {
	if (depth == mgen::DEEP)
		m_loser._setAllFieldsSet(state, mgen::DEEP);
...
bool GameOver::_isLoserSet(const mgen::FieldSetDepth depth) const {
	if (depth == mgen::SHALLOW) {
		...
		return _m_loser_isSet && mgen::validation::validateFieldDeep(getLoser());
	}
}
...
bool GameOver::_validate(const mgen::FieldSetDepth depth) const { 
	if (depth == mgen::SHALLOW) {
		...
		return true
				&& (!_isWinnerSet(mgen::SHALLOW) || _isWinnerSet(mgen::DEEP))
				&& (!_isLoserSet(mgen::SHALLOW) || _isLoserSet(mgen::DEEP))
...
const std::string& GameOver::_type_name() {
	static const std::string out("se.gigurra.aichallenge.games.go.GameOver");
	return out;
}

const std::vector<long long>& GameOver::_type_ids() {
	static const std::vector<long long> out = mgen::make_vector<long long>() << 1369439193061359418LL << 7939421773867622329LL << 5106868930527964703LL << 2162097505141779409LL;
	return out;
}

const std::vector<short>& GameOver::_type_ids_16bit() {
	static const std::vector<short> out = mgen::make_vector<short>() << -15038 << -5220 << -13325 << -494;
	return out;
}

const std::vector<std::string>& GameOver::_type_names() {
	static const std::vector<std::string> out = mgen::make_vector<std::string>() << "se.gigurra.aichallenge.Message" << "se.gigurra.aichallenge.GameMessage" << "se.gigurra.aichallenge.games.go.GoMessage" << "se.gigurra.aichallenge.games.go.GameOver";
	return out;
}

const std::vector<std::string>& GameOver::_type_ids_16bit_base64() {
	static const std::vector<std::string> out = mgen::make_vector<std::string>() << "xUI" << "65w" << "y/M" << "/hI";
	return out;
}

const std::string& GameOver::_type_ids_16bit_base64_string() {
	static const std::string out("xUI65wy/M/hI");
	return out;
}

const std::string& GameOver::_type_id_16bit_base64() {
	static const std::string out("/hI");
	return out;
}
